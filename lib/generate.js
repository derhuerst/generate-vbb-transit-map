...
const computeStationSize = (station, edgesAt) => {
	const byTarget = Object.create(null)
	for (let e of edgesAt) {
		byTarget[e.target] = byTarget[e.target] ? byTarget[e.target] + 1 : 1
	}
	let max = 0
	for (let t in byTarget) {
		if (byTarget[t] > max) max = byTarget[t]
	}
	return .1 + max * .015
}
...
const renderStations = (edges, reportBbox) => (station) => {
	...
	const radius = computeStationSize(station, edgesAt)
